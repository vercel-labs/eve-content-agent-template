 ...
 * root's skills, connections, or tools. It therefore judges only what the root packs into
 * `message`: the surface, the draft, and the active style rubric. That isolation is the point —
 * a reviewer that never saw the source or the drafting reasoning catches the voice drift,
 * AI-tells, and spec misses that self-review rationalizes away. It complements
 * `lint_against_style` (a deterministic banned-words floor) by judging the qualitative rubric a
 * regex cannot.
 *
...
export default defineAgent({
  description:
    ...
    "caller passes the surface, the draft, and the rubric in the message; returns a verdict.",
